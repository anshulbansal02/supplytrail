...
class Supply extends Contract {
    ...
    async initLedger(ctx) {
        ...
        for (let i = 0; i < admins.length; i++) {
            ...
            await ctx.stub.putState(
                "admin" + (i + 1),
                Buffer.from(JSON.stringify(admins[i]))
            );
    ...
    async queryAllProduct(ctx, manufacturerId) {
        ...
        if (!manufacturerId) {
            filterFunc = (product) =>
                product.status !== "fault" && product.quantity != 0;
        } else
            filterFunc = (product) => product.manufacturerId === manufacturerId;
        ...
        for (let i = 0; i < productCounter; i++) {
            ...
            if (filterFunc(productAsJson)) {
                const manufacturerAsBytes = await ctx.stub.getState(
                    productAsJson.manufacturerId
                );
                const manufacturerAsJSON = await JSON.parse(
                    manufacturerAsBytes.toString()
                );
                productAsJson.manufacturer = manufacturerAsJSON;

                if (!!productAsJson.markedFaultBy) {
                    const markerAsBytes = await ctx.stub.getState(
                        productAsJson.markedFaultBy
                    );
                    const markerAsJson = await JSON.parse(
                        markerAsBytes.toString()
                    );
    ...
    async getUserObj(ctx, batchAsJSON) {
        ...
        if (batchAsJSON.delivererId !== "") {
            const delivererAsBytes = await ctx.stub.getState(
                batchAsJSON.delivererId
            );
            const delivererAsJSON = await JSON.parse(
                delivererAsBytes.toString()
            );
    ...
    async registerBatchOrder(ctx, productId, retailerId, quantity) {
        ...
        if (productAsJson.status === "fault") {
            return shim.error(
                `Product ${productId} could not be ordered as it is fault`
            );
        }
    ...
    async markBatchFault(ctx, batchId, markedBy) {
        ...
        await ctx.stub.putState(
            batchId,
            Buffer.from(JSON.stringify(batchAsJson))
        );
    ...
    async transferToDeliverer(ctx, batchId, userId) {
        ...
        let batchAsJson = await JSON.parse(await batchAsBytes.toString());
        if (
            batchAsJson.status === "approve-invitation-by-deliverer" &&
            batchAsJson.manufacturerId === userId
        ) {
    ...
    async hashPassword(password) {
        return crypto
            .createHash("sha256", "hashing_secret")
            .update(password)
            .digest("hex");
    }
    ...
    async getCounterForRegister(ctx, counterType) {
        ...
        await ctx.stub.putState(
            "counters",
            Buffer.from(JSON.stringify(counterAsJson))
        );
    ...
    async delivererConfirmTransfer(ctx, batchId, userId) {
        ...
        if (
            batchAsJson.status === "transferred-to-deliverer" &&
            batchAsJson.delivererId === userId
        ) {
    ...
    async retailerConfirmTransfer(ctx, batchId, userId) {
        ...
        let batchAsJson = await JSON.parse(await batchAsBytes.toString());
        if (
            batchAsJson.status === "transferred-to-retailer" &&
            batchAsJson.retailerId === userId
        ) {
    ...
    async markProductFault(ctx, productId, manufacturerId) {
        ...
        await ctx.stub.putState(
            productId,
            Buffer.from(JSON.stringify(productAsJson))
        );
    ...
    async approveBatchOrder(ctx, batchId, userID) {
        ...
        let batch = await JSON.parse(
            await (
                await this.queryBatch(ctx, batchId)
            ).payload
        );
        ...
        const product = await JSON.parse(
            await (
                await this.queryProduct(ctx, batch.productId)
            ).payload
        );
    ...
    async approveInvitation(ctx, batchId, action, userID) {
        ...
        if (batch.delivererId !== userID) return shim.error("Wrong deliverer");
        if (action == "approved") {
    ...
    async getAllBatches(ctx, userID = "all") {
        ...
        for await (const { key, value } of ctx.stub.getStateByRange("", "")) {
            ...
            var flag = false;
            if (userID == "all") flag = true;
            else {
                const user = await JSON.parse(
                    (
                        await this.queryUser(ctx, userID)
                    ).payload
                );
                if (
                    user.userType == "deliverer" &&
                    record.delivererId == userID
                )
                    flag = true;
    ...
    async reportFaultBatch(ctx, batchId, userID) {
        ...
        for (let i in allResults) {
            if (allResults[i].productId == batch.productId) {
                ...
                await ctx.stub.putState(
                    allResults[i].batchId,
                    Buffer.from(JSON.stringify(allResults[i]))
                );
